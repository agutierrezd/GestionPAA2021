...
function GetLWWhere($field, $ptype, $table = "")
{
	...
	if(!$table)
		$table = $strTableName;
		if($table=="q_reporte_001_sector" && $field=="UEJ")
	{
		$strWhere = " seguimiento = 0";
		return $strWhere;
	}
		if($table=="paa_master" && $field=="sys_status")
	{
		...
		if($table=="paa_master_f" && $field=="sys_status")
	{
		$strWhere = " status_id = 0";
		return $strWhere;
	}
...
function GetDefaultValue($field, $ptype, $table="")
{
				...
				if($table=="global_users" && $field=="groupid")
	{
		return 1;
				...
				if($table=="001_rubro" && $field=="FUENTE")
	{
		return Nación;
	}
				if($table=="001_rubro" && $field=="TIPO")
	{
		return 1;
	}
				if($table=="002_producto" && $field=="TIPO")
	{
		return 2;
	}
				if($table=="001_rubro_f" && $field=="FUENTE")
	{
		return Nación;
	}
				if($table=="001_rubro_f" && $field=="TIPO")
	{
		return 1;
	}
				if($table=="002_producto_f" && $field=="TIPO")
	{
		return 2;
	}
				if($table=="paa_master" && $field=="UNIDAD_CONTRATACION")
	{
		return PAA;
				...
				if($table=="paa_master" && $field=="RESPONSABLE_TELEFONO")
	{
		return 6067676;
	}
				if($table=="paa_master" && $field=="sys_user")
	{
		return $_SESSION["UserID"];
				...
				if($table=="paa_master" && $field=="sys_date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
				if($table=="paa_master_f" && $field=="UNIDAD_CONTRATACION")
	{
		return PAA;
	}
				if($table=="paa_master_f" && $field=="RESPONSABLE_TELEFONO")
	{
		return 6067676;
	}
				if($table=="paa_master_f" && $field=="sys_user")
	{
		return $_SESSION["UserID"];
	}
				if($table=="paa_master_f" && $field=="sys_date")
	{
		return strftime("%Y-%m-%d %H:%M:%S");
	}
	return "";
