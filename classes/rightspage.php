...
class RightsPage extends ListPage
{
	...
	function fillGroupsArr()
	{
		...
		$sql = "select ". 
			...
			." from ". 
			$grConnection->addTableWrappers( "gestionpaa_ver_103_uggroups" ) .
	...
	function getRights()
	{
		...
		$sql = "select ". $this->connection->addFieldWrappers( "GroupID" )
			...
			." from ". $this->connection->addTableWrappers( "gestionpaa_ver_103_ugrights" )
	...
	function updateTablePermissions( $table, $group, $tableRights )
	{
		...
		$rightWTableName = $this->connection->addTableWrappers( "gestionpaa_ver_103_ugrights" );
